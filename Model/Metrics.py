        ...
        for i in range(len(query)):
            if query[i] in dict_response_ranking:
                ...
                if Metrics.one_hot_vec2index(label[i]) == target_dict.get_index('1'):  # 如果是正确答案
                    query_has_best_response[query[i]] = 'yes'
                ...
                if Metrics.one_hot_vec2index(label[i]) == target_dict.get_index('1'):  # 如果是正确答案
                    ...
                    query_has_best_response[query[i]] = 'no'
        mrr = 0
        drop_query_counter = 0
        for key in dict_response_ranking.keys():
            if query_has_best_response[key] != 'yes':
                drop_query_counter += 1
                continue
            ...
            for rank in rank_list:
                if rank > rank_list[0]:
                    counter += 1
            mrr += 1 / counter
        mrr = mrr / (len(dict_response_ranking) - drop_query_counter)
        return mrr
